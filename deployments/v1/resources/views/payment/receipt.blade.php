...
<head>
    ...
    <title>
        ...
        @elseif($payment->status === 'CANCELLED') Payment Cancelled
        @else Payment Failed @endif - DigiMart Solutions
    </title>
...
<body>
    <div class="container">
        <div class="success-icon" style="background: {{ $payment->status === 'SUCCESS' ? '#f0fdf4' : ($payment->status === 'CANCELLED' ? '#fff7ed' : '#fef2f2') }}; border-color: {{ $payment->status === 'SUCCESS' ? '#86efac' : ($payment->status === 'CANCELLED' ? '#fed7aa' : '#fecaca') }};">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="status-message">
                    @if($payment->status === 'SUCCESS') Your transaction has been processed successfully
                    @elseif($payment->status === 'CANCELLED') The payment process was cancelled by the user
                    @else There was an error processing your transaction @endif
                </div>
            </div>
            ...
            <div class="info-banner">
                ...
                <div class="info-text">
                    ...
                    @endphp
                    @if($payment->status === 'PENDING' && (app()->isLocal() || $isLocalHost))
                    <div style="margin-top: 10px; border-top: 1px solid #fde68a; pt-2;">
                        ...
                        <a href="{{ route('pay.sync', $payment->order_id) }}" class="btn" style="background: #f59e0b; color: white; margin-top: 8px; padding: 6px 12px; font-size: 12px; width: auto;">Confirm Payment Manually</a>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="details-section">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value">
                        <span class="status-badge" style="background: {{ $payment->status === 'SUCCESS' ? '#f0fdf4' : ($payment->status === 'CANCELLED' ? '#fff7ed' : '#fef2f2') }}; color: {{ $payment->status === 'SUCCESS' ? '#16a34a' : ($payment->status === 'CANCELLED' ? '#ea580c' : '#dc2626') }};">
            ...
            <div class="actions">
                ...
                <a href="/" class="btn btn-secondary">Make Another Payment</a>
            </div>
        </div>
    ...
    <script>
        ...
        function downloadReceipt() {
            ...
            html2canvas(receipt, {
                ...
                link.download = 'Receipt-{{ $payment->order_id }}.jpg';
    ...
    <div id="receipt-card" style="display: none; position: fixed; top: -9999px; left: -9999px; width: 400px; background: white; padding: 40px; border-radius: 12px; font-family: 'Inter', sans-serif; color: #0a2540;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            <div style="font-size: 36px; font-weight: 700; color: #635bff; letter-spacing: -1px;">{{ $payment->currency }} {{ number_format($payment->amount, 2) }}</div>
        ...
        <div style="border-top: 2px dashed #e3e8ee; border-bottom: 2px dashed #e3e8ee; padding: 20px 0; margin-bottom: 20px;">
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px;">
                ...
                <span style="font-weight: 600;">{{ $payment->updated_at->format('d M Y, h:i A') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px;">
        ...
        <div style="text-align: center; margin-top: 20px;">
            <img src="{{ asset('logo.png') }}" style="height: 24px; opacity: 0.5;">
        </div>
    </div>
</body>
</html>
