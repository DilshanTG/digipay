...
class PayHereService
{
    ...
    /**
     * Initialize service focusing on global defaults.
     * Use loadForMerchant() for transaction-specific logic.
     */
    public function __construct()
    {
        $this->isSandbox = (Setting::get('payhere_mode', $_ENV['PAYHERE_MODE'] ?? 'live') === 'sandbox');
    ...
    protected function loadGlobalConfig(): void
    {
        if ($this->isSandbox) {
            ...
            $this->merchantSecret = Setting::get('payhere_secret_sandbox_localhost'); // Default to locahost for global
    ...
    /**
     * Dynamic switching based on Merchant's sandbox mode
     */
    public function forMerchant(?Merchant $merchant): self
    {
        if (!$merchant) return $this;

        $this->isSandbox = (bool) $merchant->sandbox_mode;

        if ($this->isSandbox) {
            $this->merchantId = Setting::get('payhere_merchant_id_sandbox', $_ENV['PAYHERE_MERCHANT_ID'] ?? '');
            
            // Logic for sandbox secret selection (localhost vs hosted)
